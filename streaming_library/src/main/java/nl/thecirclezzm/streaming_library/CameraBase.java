...
import nl.thecirclezzm.streaming_library.encoder.audio.AudioEncoder;
import nl.thecirclezzm.streaming_library.encoder.audio.GetAacData;
import nl.thecirclezzm.streaming_library.encoder.input.audio.GetMicrophoneData;
import nl.thecirclezzm.streaming_library.encoder.input.audio.MicrophoneManager;
import nl.thecirclezzm.streaming_library.encoder.input.video.CameraApiManager;
import nl.thecirclezzm.streaming_library.encoder.input.video.CameraHelper;
import nl.thecirclezzm.streaming_library.encoder.input.video.CameraOpenException;
import nl.thecirclezzm.streaming_library.encoder.utils.CodecUtil;
import nl.thecirclezzm.streaming_library.encoder.video.FormatVideoEncoder;
import nl.thecirclezzm.streaming_library.encoder.video.GetVideoData;
import nl.thecirclezzm.streaming_library.encoder.video.VideoEncoder;
...
public abstract class CameraBase implements GetAacData, GetVideoData, GetMicrophoneData {
    ...
    public CameraBase(@NonNull SurfaceView surfaceView) {
        this.surfaceView = surfaceView;
        this.context = surfaceView.getContext();
        init(context);
    }

    public CameraBase(@NonNull TextureView textureView) {
    ...
    private void init(Context context) {
        cameraManager = new CameraApiManager(context);
     ...
     *
     * @param cameraFacing front or back camera. Like: {@link nl.thecirclezzm.streaming_library.encoder.input.video.CameraHelper.Facing#BACK}
     *                     {@link nl.thecirclezzm.streaming_library.encoder.input.video.CameraHelper.Facing#FRONT}
     * @param rotation     camera rotation (0, 90, 180, 270). Recommended: {@link
     *                     nl.thecirclezzm.streaming_library.encoder.input.video.CameraHelper#getCameraOrientation(Context)}
     */
    public void startPreview(@NonNull CameraHelper.Facing cameraFacing, int width, int height, int rotation) {
        previewWidth = width;
    ...
    public void startPreview(@NonNull CameraHelper.Facing cameraFacing, int width, int height) {
