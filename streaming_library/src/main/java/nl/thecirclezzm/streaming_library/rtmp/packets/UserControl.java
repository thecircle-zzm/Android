...
public class UserControl extends RtmpPacket {
    ...
    public UserControl(ChunkStreamInfo channelInfo) {
        super(new RtmpHeader(
                channelInfo.canReusePrevHeaderTx(RtmpHeader.MessageType.USER_CONTROL_MESSAGE)
                        ? RtmpHeader.ChunkType.TYPE_2_RELATIVE_TIMESTAMP_ONLY
                        : RtmpHeader.ChunkType.TYPE_0_FULL, ChunkStreamInfo.RTMP_CID_PROTOCOL_CONTROL,
    ...
    public void setEventData(int eventData) {
        if (type == Type.SET_BUFFER_LENGTH) {
            throw new IllegalStateException(
                    "SET_BUFFER_LENGTH requires two event data values; use setEventData(int, int) instead");
        }
    ...
    public void setEventData(int streamId, int bufferLength) {
        if (type != Type.SET_BUFFER_LENGTH) {
            throw new IllegalStateException("User control type "
                    + type
