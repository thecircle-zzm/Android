package nl.thecirclezzm.streaming_library.rtmp.packets;
...
import nl.thecirclezzm.streaming_library.rtmp.Crypto;
import nl.thecirclezzm.streaming_library.rtmp.Util;
...
public final class Handshake {
    ...
    private static final byte[] GENUINE_FP_KEY = {
            (byte) 0x47, (byte) 0x65, (byte) 0x6E, (byte) 0x75, (byte) 0x69, (byte) 0x6E, (byte) 0x65,
            (byte) 0x20, (byte) 0x41, (byte) 0x64, (byte) 0x6F, (byte) 0x62, (byte) 0x65, (byte) 0x20,
            (byte) 0x46, (byte) 0x6C, (byte) 0x61, (byte) 0x73, (byte) 0x68, (byte) 0x20, (byte) 0x50,
            (byte) 0x6C, (byte) 0x61, (byte) 0x79, (byte) 0x65, (byte) 0x72, (byte) 0x20, (byte) 0x30,
        ...
        if (s0 != PROTOCOL_VERSION && s0 != 72) { //check 72 to fix Ant Media Server
            if (s0 == -1) {
                ...
                throw new IOException(
                        "Invalid RTMP protocol version; expected " + PROTOCOL_VERSION + ", got " + s0);
            }
        }
    }
    ...
    public final void writeC1(OutputStream out) throws IOException {
        ...
        final int digestOffset = random.nextInt(HANDSHAKE_SIZE
                - DIGEST_OFFSET_INDICATOR_POS
                - 4
                - 8
                - SHA256_DIGEST_SIZE); //random.nextInt(DIGEST_OFFSET_INDICATOR_POS - SHA256_DIGEST_SIZE);
        ...
        Log.d(TAG, "writeC1(): digestOffsetBytes: " + Util.toHexString(
                digestOffsetBytes));  //Util.unsignedInt32ToByteArray((digestOffset % 728))));
        ...
        System.arraycopy(timeStamp, 0, partBeforeDigest, 0,
                4); // Bytes 0 - 3 bytes: current epoch timestamp
        System.arraycopy(new byte[]{(byte) 0x80, 0x00, 0x07, 0x02}, 0, partBeforeDigest, 4,
                4); // Bytes 4 - 7: Flash player version: 11.2.202.233

        // Create random bytes for the part after the digest
        byte[] partAfterDigest = new byte[HANDSHAKE_SIZE
                - absoluteDigestOffset
                - SHA256_DIGEST_SIZE]; // subtract 8 because of initial 8 bytes already written
        ...
        System.arraycopy(partAfterDigest, 0, tempBuffer, partBeforeDigest.length,
                partAfterDigest.length);
